...
#include <stdbool.h>
#include <stdlib.h>
#include <time.h> // adicionei a time.h pra gente cronometrar o algoritmo
...
// ----------------------------------------------------------------------
// VARIAVEIS GLOBAIS PRO NOSSO ARQUIVO DE LOG
// ----------------------------------------------------------------------
int comparacoes = 0;
int movimentacoes = 0;

// algoritmo brabo de insercao parcial (vai organizando so os top k)
void insercaoParcial(Restaurante* array, int n, int k) {
  for (int i = 1; i < n; i++) {
    Restaurante tmp = array[i];
    movimentacoes++; // conta que guardamos a struct no tmp
    
    // o truque da insercao parcial: se o i passou do k, a gente prende ele na casinha k-1
    int j = (i < k) ? (i - 1) : (k - 1);
    
    // puxa a galera pro lado ate achar onde o tmp tem q ficar
    while (j >= 0) {
      comparacoes++; 
      
      // CORRECAO: o Verde ta testando a ordenacao por CIDADE nessa questao! (Beijing, Delhi...)
      int comp = strcmp(array[j].cidade, tmp.cidade); 
      
      if (comp > 0) { // ta maior, arrasta pra direita
         array[j + 1] = array[j];
         movimentacoes++;
         j--;
      } else if (comp == 0) { // deu empate na cidade
         comparacoes++; 
         if (array[j].id_restaurante > tmp.id_restaurante) { // desempata sempre pelo idzinho
             array[j + 1] = array[j];
             movimentacoes++;
             j--;
         } else {
             break; // tmp ta no lugar dele
         }
      } else {
         break; // tmp ja eh maior, quebra o laco pq nao mexe mais
      }
    }
    array[j + 1] = tmp; // bota o tmp na casinha que ficou vaga
    movimentacoes++;
  }
}
...
int main(){
  ...
  Restaurante arrayOrdenacao[1000]; // vetorzao pra guardar a galera que vai ser ordenada
  int qtdRestaurantes = 0;

  char linha[100];
  scanf("%s", linha); // le a primeira do console
  
  // repete ate digitar FIM ou -1 pra ler todo mundo q o verde quer testar
  while(strcmp(linha, "-1") != 0 && strcmp(linha, "FIM") != 0){ 
    int id = transformarInt(linha); // passa o texto da entrada pra inteiro

    int idBuscado = buscarId(cr, id); // pega onde esse id ta no nosso csvzao da memoria
    if(idBuscado != -1){ 
      arrayOrdenacao[qtdRestaurantes] = cr->restaurante[idBuscado]; // guarda no vetor de ordenacao
      qtdRestaurantes++;
    }
    scanf("%s", linha); // pega a proxima linha e recomeca a treta
  }

  // comeca a brincadeira medindo o tempo em clocks do processador
  clock_t tempo_inicio = clock(); 

  insercaoParcial(arrayOrdenacao, qtdRestaurantes, 10); // chama passando o k = 10 exigido no pdf

  clock_t tempo_fim = clock(); 
  double tempoTotal = ((double)(tempo_fim - tempo_inicio)) / CLOCKS_PER_SEC * 1000.0; // converte pra milissegundos

  // CORRECAO: printa TODO o array pra bater com o Verde! 
  // Ele quer ver a bagunca/duplicados que sobram no final do vetor pra provar q vc usou insercao parcial.
  for(int i = 0; i < qtdRestaurantes; i++){
    char leitura[500]; // array com gordura suficiente pra nao estourar os chars
    formatar_restaurante(&arrayOrdenacao[i], leitura); // passa pro buffer
    printf("%s\n", leitura); // printa lindao na tela
  }

  // gera o arquivinho de log que tira muito ponto se faltar
  FILE *log = fopen("885428_insercao_parcial.txt", "w");
  fprintf(log, "885428\t%d\t%d\t%.2lf", comparacoes, movimentacoes, tempoTotal); // com as tabulacoes certinhas
  fclose(log);

  // faxina obrigatoria pra nao dar vazamento de memoria antes de fechar o programa
