...
// criacao do tad de lista de verdade q o verde vai testar
class MinhaLista {
  private Restaurante[] array;
  private int n; // a gente usa n pra contar qts tem dentro msm

  // a lista inicializa com espaco de folga
  public MinhaLista(int capacidade) {
    array = new Restaurante[capacidade];
    n = 0;
  }

  // II - bota na primeira casa
  public void inserirInicio(Restaurante r) {
    for (int i = n; i > 0; i--) {
      array[i] = array[i - 1]; // empurra td mundo uma casa pra frente
    }
    array[0] = r; // deixa a zero livre pra ele
    n++;
  }

  // IF - bota na ultima e ja era
  public void inserirFim(Restaurante r) {
    array[n] = r;
    n++;
  }

  // I* - bota no lugar especifico 
  public void inserir(Restaurante r, int pos) {
    for (int i = n; i > pos; i--) {
      array[i] = array[i - 1]; // empurra td mundo ate chegar na posicao
    }
    array[pos] = r; // toma o lugar da posicao
    n++;
  }

  // RI - tira o primeiro e avisa a galera
  public Restaurante removerInicio() {
    Restaurante excluido = array[0]; // salva pq a gente tem q printar ele dps
    for (int i = 0; i < n - 1; i++) {
      array[i] = array[i + 1]; // puxa todo mundo de volta uma casa 
    }
    n--; // desconta da chamada geral
    return excluido;
  }

  // RF - tira o ultimo direto 
  public Restaurante removerFim() {
    n--;
    return array[n]; // salva pra printar dps
  }

  // R* - remove de uma casa solta q a gnt mandou
  public Restaurante remover(int pos) {
    Restaurante excluido = array[pos]; 
    for (int i = pos; i < n - 1; i++) {
      array[i] = array[i + 1]; // puxa todo mundo de volta cobrindo o buraco
    }
    n--;
    return excluido;
  }

  // CORRIGIDO: mostra td no final limpo
  public void mostrar() {
    for (int i = 0; i < n; i++) {
      System.out.println(array[i].formatar()); // tirei os indexadores [i] pq o verde tava surtando
    }
  }
}
...
public class Lista {
  public static void main(String[] args) throws Exception {
    ...
    ColecaoRestaurantes cr = ColecaoRestaurantes.lerCsv(); // da o load da base
    
    MinhaLista lista = new MinhaLista(500); // garante uma lista q n estoura
    
    String linha = sc.next(); 
      
    // carrega a primeira etapa de insercoes na lista toda vez 
    while(linha.compareTo("-1") != 0) { 
      int id = Integer.parseInt(linha); 
      Restaurante r = cr.buscarPorId(id); 
      if(r != null) { 
        lista.inserirFim(r); // insere de base
      }
      linha = sc.next(); 
    }
    
    int quantidadeOps = sc.nextInt(); // quantas ordens vamos receber agora
    
    for(int i = 0; i < quantidadeOps; i++) { // repete de acordo com a ordem
      String comando = sc.next(); // ve qual a letra do verde 
      
      // CORRIGIDO: em todos os prints de (R) tirei o espaco!
      if(comando.equals("II")) { // insere comeco
        int id = sc.nextInt();
        Restaurante r = cr.buscarPorId(id);
        lista.inserirInicio(r);
        
      } else if(comando.equals("IF")) { // insere final
        int id = sc.nextInt();
        Restaurante r = cr.buscarPorId(id);
        lista.inserirFim(r);
        
      } else if(comando.equals("I*")) { // insere em tal canto
        int pos = sc.nextInt();
        int id = sc.nextInt();
        Restaurante r = cr.buscarPorId(id);
        lista.inserir(r, pos);
        
      } else if(comando.equals("RI")) { // remove comeco e printa o coitado q saiu
        Restaurante r = lista.removerInicio();
        System.out.println("(R)" + r.getNome());
        
      } else if(comando.equals("RF")) { // remove fim e printa
        Restaurante r = lista.removerFim();
        System.out.println("(R)" + r.getNome());
        
      } else if(comando.equals("R*")) { // remove aleatorio e printa
        int pos = sc.nextInt();
    ...
    lista.mostrar(); // no final printa todo mundo formatado
    sc.close(); 
  }
}