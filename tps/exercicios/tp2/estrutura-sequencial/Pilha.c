...
// cria a logica da Pilha em C agorinha pro exercicio 12
typedef struct {
  Restaurante array[500]; // ja boto logo 500 pq nunca da ruim
  int topo; // e a variavel pra saber onde ta acabando
} MinhaPilha;

// joga o valor sempre onde tem lugar vago
void empilhar(MinhaPilha* pilha, Restaurante r) {
  pilha->array[pilha->topo] = r; 
  pilha->topo++; // aumenta o tamanho
}

// puxa o ultimo que vc colocou
Restaurante desempilhar(MinhaPilha* pilha) {
  pilha->topo--; // diminiu do total global
  return pilha->array[pilha->topo]; // pega de volta do array
}

// CORRIGIDO: exibe do topo pra baixo MAS sem os indices [i] malditos
void mostrar(MinhaPilha* pilha) {
  for (int i = pilha->topo - 1; i >= 0; i--) { // laco decrescente indo do topo pra base da pilha
    char leitura[300];
    formatar_restaurante(&(pilha->array[i]), leitura); // converte pra string final
    printf("%s\n", leitura); // solta o resultado sem index nenhum
  }
}
...
int main() {
  ...
  MinhaPilha pilha; // instancia a nossa estrutura de pilha
  pilha.topo = 0; // zera contador pra comecar a brincar

  char linha[100];
  scanf(" %[^\n\r]", linha); // evita dores de cabeca com enter nojento
  
  while(strcmp(linha, "-1") != 0) { // alimenta de base antes do verde testar de vdd
    int id = transformarInt(linha); 
    int idBuscado = buscarId(cr, id); 
    
    if(idBuscado != -1) { 
      empilhar(&pilha, cr->restaurante[idBuscado]); // usa os dados prontos no push padrao
    }
    scanf(" %[^\n\r]", linha); 
  }

  int quantOps;
  scanf("%d", &quantOps); // le quantas remocoes/insercoes vem ai
  
  for(int i = 0; i < quantOps; i++) {
    char comando;
    scanf(" %c", &comando); // pega a letrinha magica
    
    if (comando == 'I') { // I de Insert / Push
      int id;
      scanf("%d", &id); // se for de inserir a gnt tem q ler ql o id logico
      int idBuscado = buscarId(cr, id);
      if(idBuscado != -1){
        empilhar(&pilha, cr->restaurante[idBuscado]); // sobe o maninho novo 
      }
    } else if (comando == 'R') { // R de Popzinho basico
      Restaurante excluido = desempilhar(&pilha); // puxa la de cima da panela
      printf("(R)%s\n", excluido.nome); // CORRIGIDO: acusa no console quem saiu SEM espaco
    }
  }

  mostrar(&pilha); // da display limpo 
  
  // a de lei pra fechar bonito 
