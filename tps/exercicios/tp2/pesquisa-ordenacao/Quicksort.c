...
// separei a logica da troca pq o quicksort usa ela toda hora
void realizar_troca(Colecao_Restaurante* arrayOrdenacao, int esq, int dir) {
  Restaurante temp = arrayOrdenacao->restaurante[esq];
  arrayOrdenacao->restaurante[esq] = arrayOrdenacao->restaurante[dir];
  arrayOrdenacao->restaurante[dir] = temp; // copia cruzada
  num_movimentacoes += 3; 
}

// quicksort chumbado q chama recursao
void quicksort(Colecao_Restaurante* arrayOrdenacao, int esq, int dir) {
  int i = esq, j = dir;
  Restaurante pivo = arrayOrdenacao->restaurante[(esq + dir) / 2]; // eu escolho o pivo sempre cravado no meio
  
  while (i <= j) {
    // anda pra direita procurando alguem maior pra trocar
    // DESEMPATE CORRIGIDO: compara os nomes em ordem alfabetica se as avaliacoes forem iguais
    while (arrayOrdenacao->restaurante[i].avaliacao < pivo.avaliacao || 
          (arrayOrdenacao->restaurante[i].avaliacao == pivo.avaliacao && 
           strcmp(arrayOrdenacao->restaurante[i].nome, pivo.nome) < 0)) {
      i++;
      num_comparacoes++; 
    }
    num_comparacoes++; // soma pq avaliou a ultima vez e falhou
    
    // anda pra esquerda procurando alguem menor pra trocar
    while (arrayOrdenacao->restaurante[j].avaliacao > pivo.avaliacao || 
          (arrayOrdenacao->restaurante[j].avaliacao == pivo.avaliacao && 
           strcmp(arrayOrdenacao->restaurante[j].nome, pivo.nome) > 0)) {
      j--;
      num_comparacoes++; 
    }
    num_comparacoes++; 
    
    // se os dois acharam candidatos faz o swap
    if (i <= j) {
      realizar_troca(arrayOrdenacao, i, j);
      i++;
      j--;
    }
  }
  
  // faz o quick nos galhos q sobraram ate acabar o array
  if (esq < j) quicksort(arrayOrdenacao, esq, j);
  if (i < dir) quicksort(arrayOrdenacao, i, dir);
}
...
int main() {
  ...
  char linha[100];
  scanf(" %[^\n\r]", linha); // puxa pulando quebra de linha de antes
  
  while(strcmp(linha, "-1") != 0) { // repete de costume o carregamento inicial
  ...
  clock_t inicio = clock(); // dispara timer
  
  quicksort(arrayOrdenacao, 0, arrayOrdenacao->tamanho - 1); // chama a braba passando inicio e fim absolutos
  ...
  for(int i = 0; i < arrayOrdenacao->tamanho; i++){ // descarrega na saida padrao
    char leitura[300];
