...
public class Sequencial {
  public static void main(String[] args) throws Exception {
    ...
    ColecaoRestaurantes cr = ColecaoRestaurantes.lerCsv(); // carrega base gigante toda
    
    Restaurante[] arrayPesquisa = new Restaurante[500]; // cria array menor so pros elementos que vamos pesquisar
    int n = 0; 
    
    String linha = sc.nextLine().trim(); // le a primeira linha tirando espacos em branco da borda
      
    while(!linha.equals("-1")) { // repete ate digitar -1 pra primeira parte do arquivo
      int id = Integer.parseInt(linha); 
      Restaurante r = cr.buscarPorId(id); 
      if(r != null) { 
        arrayPesquisa[n] = r; // bota os pesquisados no array menor
        n++;
      }
      linha = sc.nextLine().trim(); // le a proxima pra testar o laço
    }
    
    int numComparacoes = 0;
    long inicio = System.currentTimeMillis(); 

    linha = sc.nextLine().trim(); // le a primeira linha dos nomes de pesquisa
    while(!linha.equals("FIM")) { // repete ate digitar FIM maiusculo
      boolean achou = false; // flag padrao sempre recomeca falso
      
      for(int i = 0; i < n; i++) { // vai do inicio ate o final do array procurando a palavra
        numComparacoes++; // anota q fez uma comparacao
        if(arrayPesquisa[i].getNome().equals(linha)) { // compara se e igual a linha de entrada
          achou = true; // se achar muda a variavel pra true e para o laco
          break; // nao precisa continuar se ja achou
        }
      }
      
      if(achou) System.out.println("SIM"); // se acabou o laco e for true da print
      else System.out.println("NAO");
      
      linha = sc.nextLine().trim(); // pega a proxima pesquisa antes de acabar
    }
    
    long fim = System.currentTimeMillis();
    double tempoFinal = (fim - inicio) / 1000.0;

    FileWriter log = new FileWriter("matricula_sequencial.txt"); // cria arquivo exigido
    log.write("SuaMatriculaAqui\t" + tempoFinal + "\t" + numComparacoes); // salva as info
    log.close(); // ja era
