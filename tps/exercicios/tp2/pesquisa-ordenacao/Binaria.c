...
void ordenar_por_selecao(Colecao_Restaurante* arrayOrdenacao) { 
  for (int i = 0; i < (arrayOrdenacao->tamanho - 1); i++) {
    int menor = i; 
    for (int j = (i + 1); j < arrayOrdenacao->tamanho; j++) {
      if (strcmp(arrayOrdenacao->restaurante[menor].nome, arrayOrdenacao->restaurante[j].nome) > 0) {
        menor = j; // pega o menor de todos e salva a posicao
      }
    }
    
    if(menor != i){ // troca e joga o menor pro comeco
      Restaurante aux = arrayOrdenacao->restaurante[menor];
      arrayOrdenacao->restaurante[menor] = arrayOrdenacao->restaurante[i];
      arrayOrdenacao->restaurante[i] = aux;
    }
  }
}

int main() {
  Colecao_Restaurante* cr = ler_csv(); // abre tudo q tem no tmp
  
  Colecao_Restaurante* arrayPesquisa = (Colecao_Restaurante*) malloc(sizeof(Colecao_Restaurante));
  arrayPesquisa->tamanho = 0;
  arrayPesquisa->restaurante = (Restaurante*)malloc(500 * sizeof(Restaurante)); // aloca na memoria espaco de sobra

  char linha[100];
  scanf(" %[^\n\r]", linha); // le a linha toda pra nao bugar nos espacos entre as palavras
  
  while(strcmp(linha, "-1") != 0) { // repete ate digitar -1 pra puxar ids
    int id = transformarInt(linha); 
    int idBuscado = buscarId(cr, id); 
    
    if(idBuscado != -1) { 
      arrayPesquisa->restaurante[arrayPesquisa->tamanho] = cr->restaurante[idBuscado]; // guarda no nosso array
      arrayPesquisa->tamanho++;
    }
    scanf(" %[^\n\r]", linha); // repete o scaneamento
  }

  ordenar_por_selecao(arrayPesquisa); // e obrigado ordenar antes pra binaria funcionar direito

  clock_t inicio = clock(); // comeca a conta aqui
  
  scanf(" %[^\n\r]", linha); // pega a primeira frase pra pesquisar
  while(strcmp(linha, "FIM") != 0) { // repete ate digitar FIM
    bool achou = false; // padrao sempre false ate que se prove
    int dir = arrayPesquisa->tamanho - 1; // ponteiro da direita sempre do final do vetor
    int esq = 0; // ponteiro do inicio sempre o zero
    int meio;
    
    while (esq <= dir) { // laco da binaria em si
      meio = (esq + dir) / 2; // acha o meio dividindo oq sobrar por dois
      
      num_comparacoes++; 
      int comp = strcmp(linha, arrayPesquisa->restaurante[meio].nome); // ve se o nome e igual
      
      if (comp == 0) {
        achou = true; // se bateu achou
        break; // break pra sair o laco logo
      } else if (comp > 0) {
        num_comparacoes++; 
        esq = meio + 1; // se for maior vai pra direita andando a esquera
      } else {
        num_comparacoes++; 
        dir = meio - 1; // se for menor vai pra esquerda andando a direita
      }
    }
  ...
  clock_t fim = clock(); // encerra relogio
  double tempoExecucao = ((double)(fim - inicio)) / CLOCKS_PER_SEC;
  
  FILE *log = fopen("matricula_binaria.txt", "w"); // abre arquivo modo write
  fprintf(log, "SuaMatriculaAqui\t%f\t%d", tempoExecucao, num_comparacoes); // insere os dados
  fclose(log); 

  for (int i = 0; i < cr->tamanho; i++) { // corre pela fila limpando rastro
    liberar_restaurante(&cr->restaurante[i]); 
  }
  free(cr->restaurante); // solta string giga
  free(cr); 
  
  free(arrayPesquisa->restaurante);
  free(arrayPesquisa);
  
  return 0; // fim de papo
}