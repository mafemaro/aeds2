...
class ColecaoArray { 
  ...
  // construtor desse array novo de ordenacao
  public ColecaoArray(int capacidade) {
    array = new Restaurante[capacidade]; // cria o vetor que vai organizar tudo
    tamanho = 0;
  }

  public void inserirFim(Restaurante r) { 
    array[tamanho] = r; // coloca sempre na ultima posicao disponivel
    tamanho++;
  }

  public void ordenarPorInsercao() {
    for (int i = 1; i < tamanho; i++) { // laco principal que vai inserindo elemento a elemento
      Restaurante tmp = array[i]; // pega o elemento atual pra comparar com os anteriores
      int j = i - 1;

      numComparacoes++; // soma comparacao do laco while antes de entrar
      // enquanto for maior vai jogando pra frente tudo que e maior
      while ((j >= 0) && (array[j].getCidade().compareTo(tmp.getCidade()) > 0)) {
        array[j + 1] = array[j]; // empurra pra frente o valor
        numMovimentacoes++; 
        j--; // anda pra tras no vetor
        if(j >= 0) numComparacoes++; // se nao acabou o array soma outra comparacao
      }
      array[j + 1] = tmp; // insere o valor na posicao livre final
      numMovimentacoes++;
    }
  }

  public void imprimirTodos() {
    for(int i = 0; i < tamanho; i++){ // vai do inicio ate o final mostrando
      System.out.println(array[i].formatar()); // imprime o formatadao completo
    }
  }
}
...
public class Insercao {
  public static void main(String[] args) throws Exception {
    ...
    ColecaoRestaurantes cr = ColecaoRestaurantes.lerCsv(); // chama e le a base de csv
    
    ColecaoArray arrayOrdenacao = new ColecaoArray(500); // cria um array so pros q vierem
    
    String linha = sc.next(); // le a primeira linha da entrada padrao
      
    while(linha.compareTo("-1") != 0) { // repete ate digitar -1
      int id = Integer.parseInt(linha); // passa o texto pro numero id
      Restaurante r = cr.buscarPorId(id); // pega o id na base inteira
      
      if(r != null) { 
        arrayOrdenacao.inserirFim(r); // insere de um a um no array novo
      }
      linha = sc.next(); // chama a proxima linha e repete
    }
    
    long inicio = System.currentTimeMillis(); // comeca a checar o tempo da logica pura
    
    arrayOrdenacao.ordenarPorInsercao(); // chama a rotina q a gente criou antes
    
    long fim = System.currentTimeMillis(); // encerra relogio
    double tempoFinal = (fim - inicio) / 1000.0; // converte pra double em segundos
    
    arrayOrdenacao.imprimirTodos(); // solta na tela

    FileWriter log = new FileWriter("matricula_insercao.txt"); // cria txt
    log.write("SuaMatriculaAqui\t" + arrayOrdenacao.numComparacoes + "\t" + arrayOrdenacao.numMovimentacoes + "\t" + tempoFinal); // monta as infos e guarda
    log.close(); // fecha o arquivo pra terminar e fechar cache

    sc.close(); // ja era scanner
  }
}