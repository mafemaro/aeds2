...
// pra desempatar a gente faz um insertion sort de ID basicao antes
void ordenar_por_id(Colecao_Restaurante* arrayOrdenacao) {
  for (int i = 1; i < arrayOrdenacao->tamanho; i++) {
    Restaurante tmp = arrayOrdenacao->restaurante[i];
    int j = i - 1;
    num_comparacoes++; 
    
    // joga os ids q sao maiores pra frente (ordem crescente de desempate)
    while ((j >= 0) && (arrayOrdenacao->restaurante[j].id_restaurante > tmp.id_restaurante)) {
      arrayOrdenacao->restaurante[j + 1] = arrayOrdenacao->restaurante[j];
      num_movimentacoes++;
      j--;
      if (j >= 0) num_comparacoes++; // se nao encostou no inicio compara de novo
    }
    arrayOrdenacao->restaurante[j + 1] = tmp;
    num_movimentacoes++;
  }
}

// acha a capacidade maxima pra criar o vetor de contador certo
int pegar_maior_capacidade(Colecao_Restaurante* arrayOrdenacao) {
  int maior = arrayOrdenacao->restaurante[0].capacidade;
  for (int i = 1; i < arrayOrdenacao->tamanho; i++) {
    num_comparacoes++; 
    if (arrayOrdenacao->restaurante[i].capacidade > maior) {
      maior = arrayOrdenacao->restaurante[i].capacidade; // substitui se achar alguem maior
    }
  }
  return maior;
}
...
// counting sort raiz 
void counting_sort(Colecao_Restaurante* arrayOrdenacao) {
  int maior = pegar_maior_capacidade(arrayOrdenacao);
  
  // aloca o array de contagem baseado no maior mais um
  int* count = (int*)calloc((maior + 1), sizeof(int));
  Restaurante* ordenado = (Restaurante*)malloc(arrayOrdenacao->tamanho * sizeof(Restaurante));

  // conta quantas vezes cada capacidade aparece
  for (int i = 0; i < arrayOrdenacao->tamanho; i++) {
    count[arrayOrdenacao->restaurante[i].capacidade]++;
  }
  
  // vai acumulando com o anterior pra saber as posicoes finais
  for (int i = 1; i <= maior; i++) {
    count[i] += count[i - 1];
  }
  
  // vai de tras pra frente pra ficar estavel (garante que o id nao embaralha de novo)
  for (int i = arrayOrdenacao->tamanho - 1; i >= 0; i--) {
    int cap = arrayOrdenacao->restaurante[i].capacidade; // pega a capacidade dele
    ordenado[count[cap] - 1] = arrayOrdenacao->restaurante[i]; // poe ele na posicao certa do array
    count[cap]--; // ja tira um do contador pra nao dar choque
    num_movimentacoes++; // isso da mov pra gente por no log
  }
  
  // transfere do novo pro velho
  for (int i = 0; i < arrayOrdenacao->tamanho; i++) {
    arrayOrdenacao->restaurante[i] = ordenado[i];
    num_movimentacoes++;
  }
  
  free(count); // joga fora da memoria
  free(ordenado);
}
...
int main() {
  ...
  arrayOrdenacao->tamanho = 0;
  arrayOrdenacao->restaurante = (Restaurante*)malloc(500 * sizeof(Restaurante)); 
  ...
  ordenar_por_id(arrayOrdenacao); // chuta o desempate logo
  counting_sort(arrayOrdenacao); // poe o counting pra organizar pelas capacidades depois
  
  clock_t fim = clock(); 
  double tempoExecucao = ((double)(fim - inicio)) / CLOCKS_PER_SEC;
  
  FILE *log = fopen("matricula_countingsort.txt", "w"); // log q o professor pede
  fprintf(log, "SuaMatriculaAqui\t%d\t%d\t%f", num_comparacoes, num_movimentacoes, tempoExecucao); 
  fclose(log); 

  for(int i = 0; i < arrayOrdenacao->tamanho; i++){ 
    char leitura[300];
    formatar_restaurante(&(arrayOrdenacao->restaurante[i]), leitura); 
    printf("%s\n", leitura); 
  }
