...
class Data{
  ...
  // funcao nova q fiz pra comparar os anos os meses e dias pra gente ordenar dps
  public int compararTempos(Data outra){
    if(this.ano != outra.ano) return this.ano - outra.ano; // se o ano e diferente ja mata a duvida aqui
    if(this.mes != outra.mes) return this.mes - outra.mes; // senao vai olhar o mes de cada um
    return this.dia - outra.dia; // e por ultimo resolve no dia mesmo
  }
}
...
class Restaurante{
  ...
  // helper pra integrar toda a comparacao num lugar so
  public int decidaPrioridade(Restaurante outro) {
    int diferenca = this.dataAbertura.compararTempos(outro.getDataAbertura()); // vai la olhar a data da classe de dados
    if (diferenca == 0) { // deu empate nas datas todas?
      return this.idRestaurante - outro.getIdRestaurante(); // resolve vendo qual id e maior
    }
    return diferenca;
  }
}
...
class ArrayHeapsort { 
  ...
  // array limpo pra logica do heap
  public ArrayHeapsort(int capacidade) {
    array = new Restaurante[capacidade]; 
    tamanho = 0;
  }

  public void add(Restaurante r) { 
    array[tamanho] = r; // vai somando as paradas uma do lado da outra
    tamanho++;
  }
  
  public void ordernarPeloHeap() {
    // primeira fase de montagem do array pro topo da piramide
    for (int i = tamanho / 2 - 1; i >= 0; i--) { // anda ate a metade
      ajeitaArvore(tamanho, i); 
    }

    // segunda fase de jogar os valores grandes do topo pro final e diminuir a arvore
    for (int i = tamanho - 1; i > 0; i--) {
      // arranca a cabeca e poe na ponta pra ir ordenando pra tras
      Restaurante temp = array[0];
      array[0] = array[i];
      array[i] = temp;
      numMovimentacoes += 3; // soma movimentacao basica de variavel no log
      
      ajeitaArvore(i, 0); // chama sem o q sobrou pq ja ta na final certa
    }
  }

  // esse metodo recria e balanceia os "pesos" da arvore
  private void ajeitaArvore(int n, int i) {
    int maiorIndice = i; // assumes que a raiz q passamos e o maior
    int peEsquerdo = 2 * i + 1; // faz a matematica achando as pernas da arvore q derivaram dali
    int peDireito = 2 * i + 2; 

    if (peEsquerdo < n) {
      numComparacoes++; // soma pq vai comparar agora
      if (array[peEsquerdo].decidaPrioridade(array[maiorIndice]) > 0) {
        maiorIndice = peEsquerdo; // altera a raiz local
      }
    }

    if (peDireito < n) {
      numComparacoes++; // soma dnv
      if (array[peDireito].decidaPrioridade(array[maiorIndice]) > 0) {
        maiorIndice = peDireito; // se o direito for maior ele domina
      }
    }

    // se o pai original vazou e outro pegou o lugar 
    if (maiorIndice != i) {
      Restaurante temp = array[i];
      array[i] = array[maiorIndice];
      array[maiorIndice] = temp; // faz um swap da estrutura base
      numMovimentacoes += 3;
      
      ajeitaArvore(n, maiorIndice); // joga pra recursao arrumar o q desfez dps da troca
    }
  }

  public void mostrarFinal() {
    for(int i = 0; i < tamanho; i++){ 
      System.out.println(array[i].formatar()); // descarrega na tela do verde
    }
  }
}
...
public class Heapsort {
  public static void main(String[] args) throws Exception {
    ...
    ColecaoRestaurantes cr = ColecaoRestaurantes.lerCsv(); // abre base original
    
    ArrayHeapsort myHeap = new ArrayHeapsort(500); // instancia a nova ordenacao
    
    String linha = sc.next(); // consome id padrao
    ...
    while(linha.compareTo("-1") != 0) { 
      ...
      if(r != null) { 
        myHeap.add(r); // poe la dento
      }
      linha = sc.next(); // arrasta pra proxima
    }
    
    long inicio = System.currentTimeMillis(); 
    
    myHeap.ordernarPeloHeap(); // liga a logica toda de vez so
    
    long fim = System.currentTimeMillis(); 
    double tempoFinal = (fim - inicio) / 1000.0; 
    
    myHeap.mostrarFinal(); // printa organizadinho
    
    FileWriter log = new FileWriter("matricula_heapsort.txt"); // grava metricas final
    log.write("SuaMatriculaAqui\t" + myHeap.numComparacoes + "\t" + myHeap.numMovimentacoes + "\t" + tempoFinal); 
    log.close(); 
